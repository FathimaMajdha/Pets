    ...
    axiosInstance
      ...
      .then((response) => {
        const dogFoodProducts = response.data.data?.filter((product) => {
          ...
          return (
            cat === "dogfoodall" || cat === "drydogfood" || cat === "wetdogfood"
          );
        });
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-4 mt-8 ml-10">
        ...
        <button className="border shadow-md px-6 py-2 rounded-[20px]">
          Puppy Food
        </button>
        <button className="border shadow-md px-6 py-2 rounded-[20px]">
          Royal Canin
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.length > 0 ? (
            products.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-lg shadow-xl overflow-hidden relative border border-gray-400"
              >
                <img
                  className="w-48 h-48 ml-32 mt-12"
                  src={item.imageUrl}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg shadow-lg w-[400px] p-6">
            ...
            <div className="text-lg font-semibold mt-2">
              Price: ₹
              {(selectedProduct.price * (selectedProduct.quantity || 1)).toFixed(2)}
            </div>
